...
class JoystickMonitor:
    def __init__(self, cfg, selected_index=None):
        _logger.debugDeep(f"Initializing JoystickMonitor with selected_index={selected_index}")
        self.cfg = cfg
        self.selected_index = selected_index
        self.selected_joystick_name = None
        self._states = {}
    ...
    def getJoystick(self):
        pygame.init()
        _logger.debug("Getting joystick instance")
        try:
        ...
        try:
            ...
            _logger.error(f"Error getting joystick: {e}")
            return None
    
    def setJoystick(self, index):
        self.selected_index = index
        _logger.debug(f"Joystick index set to {index}")
    ...
    def refresh_states(self):
        js = self.getJoystick()
        self.selected_joystick_name = js.get_name() if js else None
        _logger.debug(f"Refreshing joystick states for joystick: {self.selected_joystick_name}")
        if js is None:
            _logger.warn("No joystick available to refresh states")
            return
        for b in range(js.get_numbuttons()):
            self._states[b] = js.get_button(b)
        _logger.debug(f"Refreshed joystick button states: {self._states}")
